...
static uint32_t gsbi4_gpio_table[] = {
	GPIO_CFG(SHOOTER_U_CAM_I2C_SDA, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(SHOOTER_U_CAM_I2C_SCL, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static struct platform_device *shooter_u_console[] __initdata = {
	&ram_console_device,
};
...
static void __init size_pmem_devices(void)
{
  ...
  android_pmem_sf_pdata.size = pmem_sf_size;
#endif
...
static int pm8058_gpios_init(void)
{
	...
	struct pm8058_gpio_cfg gpio_cfgs[] = {
		...
		{ /* Audio Speaker Amplifier */
			PM8058_GPIO_PM_TO_SYS(SHOOTER_U_AUD_HP_EN),	/* 18 */
			{
				.direction	= PM_GPIO_DIR_OUT,
				.output_value	= 0,
				.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
...
{
	platform_add_devices(shooter_u_console,
			ARRAY_SIZE(shooter_u_console));
				 ...
				 char **cmdline, struct meminfo *mi)
{
	...
	printk(KERN_INFO "shooter_u_fixup:skuid=0x%x\n", skuid);
}
